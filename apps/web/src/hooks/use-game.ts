...
export function useGame(gameId: string, spectate = false) {
  ...
  useEffect(() => {
    ...
    // game:can-concealed-kong — server notifies the active player that they hold
    // 4 of a kind and can declare a concealed (hidden) kong on their turn.
    const handleCanConcealedKong = (payload: {
      seat: 0 | 1 | 2 | 3;
      tiles: import('@nanchang/shared').TileType[];
    }) => {
      const viewerSeat = useGameStore.getState().snapshot?.viewerSeat ?? null;
      if (viewerSeat === payload.seat) {
        setCanConcealedKong(payload.tiles);
      }
    };
    ...
    s.on('game:can-concealed-kong', handleCanConcealedKong);
    ...
    return () => {
      ...
      s.off('game:can-concealed-kong', handleCanConcealedKong);
  ...
  const discard = useCallback(
    ...
    [setPendingMove, selectTile, setCanTsumo, setCanAddToKong, setCanConcealedKong],
  );
  ...
  }, [setCanTsumo, setCanAddToKong, setCanConcealedKong]);
  ...
  const kongConcealed = useCallback(
    ...
    [setCanAddToKong, setCanConcealedKong],
  );
